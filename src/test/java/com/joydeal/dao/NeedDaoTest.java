...
public class NeedDaoTest {
    ...
    public void test() throws SQLException {
        ...
        Need need = new Need().setCategory(category).setDesc("我的房子要出租了，大零居，就在小米对面")
                .setIcon("111.jpg").setName("豪宅出租").setPrice(9999.99).setOwner(user).setSeller(user)
                .setTtl(1000).setStatus(Status.Valid).setZone(zone);
        dao.add(need);
        long id = dao.lastAddedId();
        need.setId(id);
        Need got = dao.get(id);
        Assert.assertEquals(need, got);
        dao.update(need.setStatus(Status.Closed));
        got = dao.get(id);
        Assert.assertEquals(need, got);
        Assert.assertEquals(need, dao.all().get(0));
